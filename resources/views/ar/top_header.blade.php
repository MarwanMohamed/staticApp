            ...
            <div class="col-lg-9 col-12 text-lg-end">
                ...
                <a href="#quote" class="btn">إحصل على عرض سعر</a>
                 ...
                 @if(request()->segment(count(request()->segments())-1) == 'ar')
                        <a href="{{ request()->root() .'/en/'  . request()->segment(count(request()->segments()))}}">EN</a>
                    @else
                        <a href="{{ request()->root() .'/en/'  . request()->segment(count(request()->segments()) -1).'/' . request()->segment(count(request()->segments()))}}">EN</a>
                    @endif
            </div>
        </div>
    </div>
